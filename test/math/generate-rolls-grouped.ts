...
import {generateRollsGrouped} from "../../src/math/generate-rolls-grouped";

function gen(groups: number[], max: number = 6, unique: boolean = true): number[][] {
    const result = [];
    for (const comb of generateRollsGrouped(groups, max, unique)) {
        result.push(comb);
    }
    return result;
}
